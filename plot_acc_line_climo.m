...
for izone=1:7
    for isim=1:length(simList)
        ...
        accFile=sprintf('/Users/sglanvil/Documents/S2S_climo_experiments/ACC_daily/%s_ACC_%s_%sseason_%s_%s.%s_s2s_data.nc',...
            var,obsName,season,timeAvg,simName,scenarioName);
...
for izone=1:7
    ...
    plot(1:46,lndVar(izone,:),'color',lineColor(2,:),'linewidth',1.5);
    area(1:46,atmVar(izone,:),'edgecolor','none','facecolor',lineColor(1,:),...
        'facealpha',0.2,'linewidth',1.5);
    plot(1:46,atmVar(izone,:),'color',lineColor(1,:),'linewidth',1.5);
    area(1:46,ocnVar(izone,:),'edgecolor','none','facecolor',lineColor(3,:),...
        'facealpha',0.2,'linewidth',1.5);   
    plot(1:46,ocnVar(izone,:),'color',lineColor(3,:),'linewidth',1.5);
    area(1:46,lndVar2(izone,:),'edgecolor','none','facecolor',lineColor(2,:),...
        'facealpha',0.2,'linewidth',2,'linestyle',':');        
    plot(1:46,lndVar2(izone,:),'color',lineColor(2,:),'linewidth',2,'linestyle',':');
    area(1:46,atmVar2(izone,:),'edgecolor','none','facecolor',lineColor(1,:),...
        'facealpha',0.2,'linewidth',2,'linestyle',':');     
    plot(1:46,atmVar2(izone,:),'color',lineColor(1,:),'linewidth',2,'linestyle',':');
...
%     plot(1:46,climoOCN(izone,:),'color',lineColor(3,:),'linewidth',2);    
%     plot(1:46,lndFULL(izone,:),'color',lineColor(2,:),'linewidth',2,'linestyle',':');        
%     plot(1:46,atmFULL(izone,:),'color',lineColor(1,:),'linewidth',2,'linestyle',':');     
%     plot(1:46,ocnFULL(izone,:),'color',lineColor(3,:),'linewidth',2,'linestyle',':');  
...
% p(3)=plot([1 45],[-100 -100],'color',lineColor(2,:),'linewidth',2); 
% p(4)=plot([1 45],[-100 -100],'color',lineColor(3,:),'linewidth',2);
% p(5)=plot([1 45],[-100 -100],'color',lineColor(1,:),'linewidth',2,'linestyle',':'); 
% p(6)=plot([1 45],[-100 -100],'color',lineColor(2,:),'linewidth',2,'linestyle',':'); 
% p(7)=plot([1 45],[-100 -100],'color',lineColor(3,:),'linewidth',2,'linestyle',':');
% legend(p,'standard','atmosphere','land',...
%     'ocean','box','off','position',[.77 .3 .2 .2],'fontsize',12);
% legend(p,'standard','climoATM','climoLND',...
%     'climoOCN','climoOCNclimoLND','climoOCNclimoATM','climoATMclimoLND',...
%     'box','off','position',[.77 .3 .2 .2],'fontsize',12);
% annotation('textbox',[.04 .1 .8 .1],'string','\bfMethod 1: climoX (solid)',...
%     'edgecolor','none','verticalalignment','bottom','fontsize',12);
% annotation('textbox',[.04 .05 .8 .1],'string','\bfMethod 2: climoYclimoZ (dashed)',...
%     'edgecolor','none','verticalalignment','bottom','fontsize',12);
