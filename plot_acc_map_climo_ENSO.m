...
titleName=sprintf('ENSO Active - Neutral (ALL seasons) %s ACC',varLong);
printName=sprintf('%s_ACC_map_specialENSO_style3_%s_figure',var,obsName);
% ------------------------- SPECIFY ABOVE -------------------------
...
for iweek=1:3
    for icase=1:3
        ax(icase)=subplot('position',subpos(iweek,:,icase));
        hold on; box on;   
